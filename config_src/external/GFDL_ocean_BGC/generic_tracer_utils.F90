...
!            implementation    = 'ocmip2',                                                     &
  ...
  subroutine g_tracer_coupler_set(g_tracer_list,IOB_struc,value)
    ...
    if(.NOT. associated(g_tracer_list)) call MOM_error(FATAL, trim(sub_name)//&
...
!    if(present(ocean_restart_file)) ocean_restart_file  = g_tracer_com%ocean_restart_file
  ...
  subroutine g_tracer_get_4D(g_tracer_list,name,member,array_ptr)
    ...
    if(.NOT. associated(g_tracer)) call MOM_error(FATAL, trim(sub_name)//&
    ...
    case ('field')
       if(associated(g_tracer%field)) then 
          ...
          call MOM_error(FATAL, trim(sub_name)//": Cannot get member variable: "//trim(name)//" % "//trim(member))
       endif
    case default 
       call MOM_error(FATAL, trim(sub_name)//": Not a known member variable: "//trim(name)//" % "//trim(member))   
    end select
  ...
  subroutine g_tracer_get_3D(g_tracer_list,name,member,array_ptr)
    ...
    case default 
       call MOM_error(FATAL, trim(sub_name)//": Not a known member variable: "//trim(member))   
    end select
  ...
  subroutine g_tracer_set_3D(g_tracer_list,name,member,array,isd,jsd,ntau)
    ...
    case ('field') 
       ...
       elseif(associated(g_tracer%field3d_ptr)) then 
          ...
          call MOM_error(FATAL, trim(sub_name)//": Cannot set member variable: "//trim(name)//" % "//trim(member))
       endif      
    case ('vmove') 
  ...
  subroutine g_tracer_set_pointer_4D(g_tracer_list,name,member,array,ilb,jlb)
    ...
    case ('field') 
       if (associated(g_tracer%field )) then
          call MOM_error(NOTE, trim(sub_name) // ": Deallocating generic tracer "//trim(name)//" % "//trim(member))
          deallocate( g_tracer%field )
       endif
       g_tracer%field  => array 
    case default 
       call MOM_error(FATAL, trim(sub_name)//": Not a supported operation for member variable: "//trim(name)//" % "//trim(member))
    end select
...
!     local parameters
    ...
    if (depth .le. 0.0) then
      call MOM_error(FATAL, trim(error_header) // ' Depth <= 0,0')
    endif
    ...
    if (k_level .gt. nk) then
      call MOM_error(FATAL, trim(error_header) // ' k_level > nk')
    endif
  ...
  subroutine g_tracer_send_diag(g_tracer_list,model_time,tau)
    ...
    do  
       ...
       elseif(associated(g_tracer%field3d_ptr)) then 
          ...
          call MOM_error(FATAL, trim(sub_name)//": Cannot send_diag field variable for "//trim(g_tracer%name) )
       endif      
  ...
  subroutine g_tracer_print_info(g_tracer_list, verbosity)
    ...
    if (verbose >= 20) then
       ...
       if (.not. fm_dump_list('/ocean_mod/namelists', recursive = .true.)) then
          call MOM_error(FATAL, trim(sub_name) // ': Problem dumping generic tracer namelists tree')
       endif
    endif
    ...
    do  
       ...
       if(g_tracer%requires_src_info) then 
          if(g_tracer%src_file .eq. 'NULL') then
              ...
              call MOM_error(FATAL, trim(sub_name) //': '//  trim(errorstring)) 
          endif
...
!          endif
    ...
    if(errorstring .ne. '') then
       ...
       call MOM_error(WARNING, trim(sub_name) // ' : there are tracers with required source properties that are not set '//&
...
!       call MOM_error(NOTE, trim(sub_name)//&
...
!    if (is_root_pe()) print*, 'MOM_missing_value ', MOM_missing_value

    if(size(axes) .eq. 3) then
!       if (is_root_pe()) print*, 'generic_tracer_utils diag_CS_ptr%axesTL ', diag_CS_ptr%axesTL%id
